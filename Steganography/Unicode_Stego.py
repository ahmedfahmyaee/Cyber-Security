...
def bit_string(text: bytes) -> str:
    ...
    for byte in text:
        temp = str(bin(byte))[2:]
        output += ((BINARY_BLOCK_SIZE - len(temp)) * '0') + temp
    return output
...
def cipher(regular_text: str, hidden_text: str) -> str:
    ...
    regular_text = list(regular_text)   # Converting the string into a list so insertion is easier

    # Hiding the unicode characters at the end of the string
    for bit in binary_string:
        regular_text.append(CIPHER_TABLE[bit])
...
if __name__ == '__main__':
    ...
    if args.decrypt:
        deciphered_text = decipher(args.text)
